...
export default function VizyonMisyon() {
  return (
    <>
      <Helmet>
        <title>Vizyon ve Misyon | Boğaziçi Grup A.Ş.</title>
        <meta name="description" content="Boğaziçi Grup'un gelecek vizyonu, sürdürülebilir inşaat hedefleri ve kurumsal misyonu hakkında bilgi edinin." />
      </Helmet>
      <SubPageLayout
        title="VİZYON & MİSYON"
        breadcrumbs={[
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }}>
            ...
            <motion.div variants={fadeUp} className="w-12 h-1 bg-[#6cbe59] mb-6" />
            <ul className="space-y-4 mb-10">
              {[
                ...
                <motion.li key={i} variants={fadeUp} className="flex items-start gap-3 text-[#555] text-[15px] leading-7">
                  <span className="w-2 h-2 bg-[#6cbe59] rounded-full shrink-0 mt-2.5" />
                  {item}
                </motion.li>
              ))}
            </ul>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.3 }}>
            ...
            <div className="mt-10 bg-[#f8f8f8] border-l-4 border-[#6cbe59] p-6">
              ...
              <Link href="/kurumsal/kalite-politikamiz" className="inline-flex items-center gap-1 text-[#6cbe59] text-xs font-bold uppercase tracking-widest hover:gap-2 transition-all" style={{ fontFamily: "'Raleway', sans-serif" }}>
