...
export default function Belgeler() {
  return (
    <>
      <Helmet>
        <title>Belgeler ve Sertifikalar | Boğaziçi Grup A.Ş.</title>
        <meta name="description" content="Boğaziçi Grup'un ISO 9001, ISO 14001 ve Ticaret Odası faaliyet belgelerini görüntüleyin veya indirin." />
      </Helmet>
      <SubPageLayout
        title="BELGELER"
        breadcrumbs={[
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16">
          {belgeler.map((belge) => (
            <motion.div key={belge.file} variants={fadeUp} className="group bg-white border border-gray-200 hover:border-[#6cbe59] transition-all shadow-sm hover:shadow-md overflow-hidden">
              ...
              <div className="p-6">
                  <h3 className="text-sm font-bold text-navy uppercase mb-2 font-display">{belge.name}</h3>
                ...
                <div className="flex items-center gap-3">
                  <a
                    ...
                    className="inline-flex items-center gap-2 bg-navy hover:bg-accent text-white text-[11px] font-bold uppercase tracking-wider px-4 py-2.5 transition-all active:scale-[0.97] font-heading"
                  >
                    <Eye size={13} /> İncele
                  </a>
                  <a
                    href={`${pdfBase}/${belge.file}`}
                    download={belge.file}
                    className="inline-flex items-center gap-2 border border-gray-300 hover:border-accent hover:text-accent text-[#555] text-[11px] font-bold uppercase tracking-wider px-4 py-2.5 transition-all active:scale-[0.97] font-heading"
                  >
